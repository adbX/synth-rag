...
1. Query a single paper (using default local ollama/llama3.2):
...
3. Query with custom model (e.g., Claude):
    python paperqa_computo.py \
        --question "What are the key findings?" \
        --model "claude-3-5-sonnet-20240620" \
        --max-sources 5

4. Query with custom ollama settings:
    python paperqa_computo.py \
...
async def query_single_paper(
    ...
    try:
        ...
        try:
            ...
            if model.startswith("ollama/"):
                result["settings"]["summary_llm"] = model
...
async def async_main(args):
    ...
    print("Paperqa Computo Query Script")
    print("=" * 50)
